...
export DB_ADDRESS=$(AWS_PROFILE=davidcsi-hotmail-private aws --region=us-east-1 rds describe-db-instances --db-instance-identifier $DB_ID | jq '.DBInstances[].Endpoint.Address' | sed 's/"//g')
...
# AWS_PROFILE=davidcsi-hotmail-private aws eks --region $AWS_REGION update-kubeconfig --name $CLUSTER_NAME
...
export SIP_PUBLIC_IP=$(AWS_PROFILE=davidcsi-hotmail-private aws --region $AWS_REGION ec2 describe-instances --filters "Name=tag:aws:autoscaling:groupName,Values=*sip-proxy*" | jq '.Reservations[].Instances[].PublicIpAddress' | sed 's/"//g' | head -n 1)
